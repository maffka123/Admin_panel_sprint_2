"""
ETL process for updating ES base in case if genre table in postgres will be changed
"""
import logging
...
class ETL:
    ...
    def __init__(self):
        ...
        e1 = self.extract_genres_from_psql(e2)
    ...
    def extract_genres_from_psql(self, data):
        """
        Coroutine that queries postgres to get genres that were updated after updated_at time in written in state
        :param data:
        :return: dataframe with genres data
        """
        while True:
            ...
            updated_at = self.state.get_state('genre')
            query = f"""
            SELECT * FROM genre 
            ...
            logger.info('extracted %s genres from postgres' % str(len(df)))
            data.send(df)
    ...
    def extract_movies_from_psql(self, data):
        ...
        while True:
            ...
            df = (yield)
            if not df.empty:
                genres_ids = df['id'].unique()
                genres_ids = ', '.join("'"+str(id)+"'" for id in genres_ids)
                query = f"""
    ...
    def transform_sql_to_es(self, data):
        ...
        while True:
            ...
            df = (yield)
            docs = []
            updated_at = self.state.get_state('genre') # just because with df it is easier
            if not df.empty:
    ...
    def load_to_es(self):
        ...
        while True:
            ...
            self.state.set_state('genre', data[1]) # update state after all is done
