"""
ETL process for updating ES base in case if person table in postgres will be changed
"""
import logging
...
class ETL:
    ...
    def __init__(self):
        ...
        e3 = self.extract_genre_from_psql(t)
        e2 = self.extract_movies_from_psql(e3)
        e1 = self.extract_person_from_psql(e2)
    ...
    def extract_person_from_psql(self, data):
        """
        Coroutine that queries postgres to get person that were updated after updated_at time in written in state
        :param data:
        ...
        while True:
            ...
            updated_at = self.state.get_state('person')
            query = f"""
            SELECT * FROM person 
            ...
            logger.info('extracted %s people from postgres' % str(len(df)))
            data.send(df)
    ...
    @coroutine
    def extract_genre_from_psql(self, data):
        """
    ...
    def transform_sql_to_es(self, data):
        ...
        while True:
            ...
            df = (yield)
            docs = []
            updated_at = self.state.get_state('person') # just because with df it is easier
            if not df.empty:
    ...
    def load_to_es(self):
        ...
        while True:
            ...
            self.state.set_state('person', data[1]) # update state after all is done
